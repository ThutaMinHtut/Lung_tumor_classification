...
FOLDER_NAME = "CGANsamplesnewdata1312BS8th3"
GENERATOR_MODEL_NAME = "best_generator_1312thBS8CGANNEW1.pth"
DISCRIMINATOR_MODEL_NAME = "best_discriminator_1312thBS8CGANNEW1.pth"

# Hyperparameters
EPOCHS = 301  
BATCH_SIZE = 8
IMAGE_SIZE = 512
LATENT_DIM = 320 #need to change G layer if want to change latent dim
NUM_CLASSES = 6  # Number of classes in dataset
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
SAVE_EVERY = 10 

os.makedirs(FOLDER_NAME, exist_ok=True)
os.makedirs("models2/best_overall", exist_ok=True)  
os.makedirs("models2/every_10_epochs", exist_ok=True)  
os.makedirs("models2/final_model", exist_ok=True) 

for class_id in range(NUM_CLASSES):
    os.makedirs(f"models2/best_class_{class_id}", exist_ok=True)
...
if __name__ == '__main__':
    ...
    dataset = CTScanDataset("GANTRAINUPDATED", transform=transform)
        ...
        # Compute the overall average MS-SSIM score across all classes (excluding None values)
            ...
            for class_label, score in msssim_scores_per_class.items():
                if score is not None and score > best_msssim_per_class[class_label]:
                    ...
                    torch.save(generator.state_dict(), f"models2/best_class_{class_label}/generator_epoch_{epoch}.pth")
                    torch.save(discriminator.state_dict(), f"models2/best_class_{class_label}/discriminator_epoch_{epoch}.pth")
            ...
            if average_msssim > best_msssim:
                ...
                torch.save(generator.state_dict(), "models2/best_overall/generator_best.pth")
                torch.save(discriminator.state_dict(), "models2/best_overall/discriminator_best.pth")
                print(f"Saved new best overall model at epoch {epoch} with MS-SSIM {average_msssim:.4f}")

            #  SAVE MODELS EVERY 10 EPOCHS
            if epoch % 10 == 0:
                torch.save(generator.state_dict(), f"models2/every_10_epochs/generator_epoch_{epoch}.pth")
                torch.save(discriminator.state_dict(), f"models2/every_10_epochs/discriminator_epoch_{epoch}.pth")
    ...
    torch.save(generator.state_dict(), "models2/final_model/generator_final.pth")
    torch.save(discriminator.state_dict(), "models2/final_model/discriminator_final.pth")
